...
print(ggplot(df3,aes(x=logTP,y=logCHLA,color=time))+
        geom_point(aes(alpha=0.5))+
        geom_smooth(method="gam")+
        labs(title="GAM"))

lm.first <- lm(logCHLA~logTP,data=df3[df3$time=="first",])
lm.last <- lm(logCHLA~logTP,data=df3[df3$time=="last",])

print(ggplot(df2,aes(x=logTP,y=logCHLA))+
        geom_point()+
        geom_abline(slope=lm2$coefficients[2],intercept=lm2$coefficients[1],color="red")+ #overall
        geom_abline(slope=lm.first$coefficients[2],intercept=lm.first$coefficients[1],color="green")+ #first
        geom_abline(slope=lm.last$coefficients[2],intercept=lm.last$coefficients[1],color="blue")+ #last
